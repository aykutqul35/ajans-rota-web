      ...
      {currentPath !== '/referanslar' && currentPath !== '/ekiplerimiz' && currentPath !== '/neden-izmir' && !currentPath.startsWith('/hizmetlerimiz/') && (
        <div className="container mx-auto px-4 mb-16">
          <div className="relative rounded-3xl overflow-hidden bg-gradient-to-r from-primary to-secondary p-[1px]">
            ...
            <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 text-center md:text-left">
              <div className="max-w-2xl text-white">
                <h3 className="text-2xl md:text-3xl font-bold mb-3">Ege'nin Dijital Büyüme Ortağıyla Tanışın</h3>
                <p className="text-slate-300 text-lg">Markanızın dijital dünyada yeni zirvelere ulaşması ve rotanızı veriye dayalı stratejilerle çizmek için hazır mısınız?</p>
              </div>
              <div>
                <a href="/iletisim" className="inline-flex items-center gap-2 bg-gradient-to-r from-primary to-secondary text-white font-bold py-4 px-8 rounded-full hover:shadow-[0_0_30px_rgba(2,132,199,0.5)] transition-all transform hover:scale-105" onClick={e => {
                  e.preventDefault();
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-16">
          ...
          <div className="flex flex-col items-start">
            <a href="/" className="inline-flex items-center gap-3 text-2xl font-bold text-white mb-6" onClick={e => {
              e.preventDefault();
              handleNavClick('home');
            }}>
            ...
            <div className="w-full bg-slate-800/50 rounded-2xl p-6 border border-slate-700/50">
              <h5 className="text-white font-bold mb-2">Büyüme Bülteni</h5>
              <p className="text-slate-400 text-xs mb-4">E-ticaret ve dijital pazarlama rehberlerimiz posta kutunuza gelsin.</p>
              <form onSubmit={handleNewsletterSubmit} className="flex gap-2">
                <input type="email" placeholder="E-posta adresiniz" value={newsletterEmail} onChange={e => setNewsletterEmail(e.target.value)} disabled={newsletterLoading} required className="flex-1 bg-slate-900/80 border border-slate-700 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-primary transition-colors" />
                <button type="submit" disabled={newsletterLoading} aria-label="Abone Ol" className="bg-primary hover:bg-primary/90 text-white rounded-xl w-12 flex items-center justify-center transition-colors disabled:opacity-50">
          ...
          {/* Links Block */}
          <div>
            <h4 className="text-white font-bold text-lg mb-6 flex items-center gap-2"><i className="fa-solid fa-briefcase text-primary/70"></i>Hizmetlerimiz</h4>
          ...
          <div>
            <h4 className="text-white font-bold text-lg mb-6 flex items-center gap-2"><i className="fa-solid fa-building text-primary/70"></i>Kurumsal</h4>
            <ul className="flex flex-col gap-3">
          ...
          <div>
            <h4 className="text-white font-bold text-lg mb-6 flex items-center gap-2"><i className="fa-solid fa-screwdriver-wrench text-primary/70"></i>Büyüme Araçları</h4>
            <ul className="flex flex-col gap-3">
        ...
        <div className="relative overflow-hidden bg-slate-800/40 border border-slate-700/50 rounded-3xl p-8 mb-16">
          {/* Dekoratif arka plan */}
          ...
          <div className="relative z-10 flex flex-wrap items-center justify-between gap-4 mb-8">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-sky-600 to-violet-600 flex items-center justify-center shadow-[0_4px_16px_rgba(2,132,199,0.3)]">
                <i className="fa-solid fa-map-pin text-white text-lg"></i>
              </div>
              <div>
                <h4 className="text-white font-bold text-lg mb-1">Hizmet Verdiğimiz Bölgeler</h4>
          ...
          <div className="relative z-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div key={ci} className="bg-slate-900/50 border border-slate-700/50 rounded-2xl p-5 transition-all duration-300 hover:bg-slate-800 hover:-translate-y-1 hover:shadow-xl" style={{ borderTop: `3px solid ${city.color}` }}>
                {/* Şehir başlığı */}
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="text-[0.65rem] font-bold text-slate-300 bg-slate-800 px-2 py-0.5 rounded-md border border-slate-700">{city.nufus}</span>
                </div>

                {/* İlçe linkleri */}
                <div className="flex flex-col gap-1.5 mb-4">
                  {city.ilceler.map((ilce, ii) => (
                    <a key={ii} href={`/dijital-ajans/${ilce.slug}`} onClick={e => { e.preventDefault(); navigate(`/dijital-ajans/${ilce.slug}`); }} className="flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-[0.8rem] font-medium text-slate-400 hover:text-white transition-colors group">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          ...
          <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 flex flex-col items-start transition-all hover:bg-slate-800">
          ...
          <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 flex flex-col items-start transition-all hover:bg-slate-800">
            ...
            <div className="text-slate-400 text-sm mb-6 flex-1 flex flex-col gap-2">
              <p><strong className="text-slate-300">Telefon:</strong> <a href={`tel:${settingsData.phone}`} className="hover:text-white transition-colors">{settingsData.phone}</a></p>
              <p><strong className="text-slate-300">E-Posta:</strong> <a href={`mailto:${settingsData.email}`} className="hover:text-white transition-colors">{settingsData.email}</a></p>
            </div>
            <div>
          ...
          <div className="bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 flex flex-col items-start transition-all hover:bg-slate-800">
            ...
            <div className="text-slate-400 text-sm mb-6 flex-1">
              ...
              {(() => {
                ...
                return (
                  <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold border ${currentStatus.status === "open" ? "bg-emerald-900/30 text-emerald-400 border-emerald-800" : "bg-slate-800 text-slate-400 border-slate-700"}`}>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-slate-800">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-500 text-xs mb-4">Dijital Pazarlama, SEO, E-Ticaret, Google, Meta, TikTok Ads Çözüm Ortağınız ; Ajans Rota.</p>
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold bg-slate-800 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700"><i className="fa-brands fa-google text-red-500"></i> Partner</span>
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold bg-slate-800 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700"><i className="fa-brands fa-meta text-blue-500"></i> Business Partner</span>
              <span className="inline-flex items-center gap-1.5 text-xs font-semibold bg-slate-800 text-slate-300 px-3 py-1.5 rounded-lg border border-slate-700"><i className="fa-brands fa-tiktok text-white"></i> Agency Partner</span>
            </div>
          </div>
          
          <div className="flex flex-col items-center md:items-end gap-6">
            <div className="flex flex-wrap items-center justify-center gap-4 text-sm text-slate-400">
              <a href="/gizlilik-politikasi" className="hover:text-white transition-colors" onClick={e => { e.preventDefault(); navigateTo('/gizlilik-politikasi'); }}>Gizlilik Politikası</a>
              <a href="/kullanim-kosullari" className="hover:text-white transition-colors" onClick={e => { e.preventDefault(); navigateTo('/kullanim-kosullari'); }}>Kullanım Koşulları</a>
              <a href="/kvkk-aydinlatma-metni" className="hover:text-white transition-colors" onClick={e => { e.preventDefault(); navigateTo('/kvkk-aydinlatma-metni'); }}>KVKK</a>
              <a href="/cerez-politikasi" className="hover:text-white transition-colors" onClick={e => { e.preventDefault(); navigateTo('/cerez-politikasi'); }}>Çerez Politikası</a>
            </div>
            <div className="flex items-center gap-4">
              <a href={settingsData.linkedin_url || "#"} className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-[#0077b5] hover:text-white transition-colors text-lg" aria-label="LinkedIn" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-linkedin-in"></i></a>
              <a href={settingsData.instagram_url || "#"} className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-gradient-to-tr hover:from-orange-500 hover:to-purple-600 hover:text-white transition-colors text-lg" aria-label="Instagram" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-instagram"></i></a>
              <a href={settingsData.whatsapp_assistant_phone ? `https://wa.me/${settingsData.whatsapp_assistant_phone}?text=${encodeURIComponent('Merhaba, web siteniz üzerinden ulaşıyorum. Bulunduğum sayfa: ' + window.location.href)}` : (settingsData.whatsapp_url || "#")} className="w-10 h-10 rounded-full bg-slate-800 flex items-center justify-center text-slate-400 hover:bg-[#25D366] hover:text-white transition-colors text-lg" aria-label="WhatsApp" target="_blank" rel="noopener noreferrer"><i className="fa-brands fa-whatsapp"></i></a>
            </div>
          </div>
        </div>
      </div>
