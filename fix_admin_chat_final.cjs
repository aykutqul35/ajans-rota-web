...
// I will just read the file, split by lines, and carefully reconstruct the activeTab === 'tickets' block.
const lines = code.split('\n');
let newLines = [];
let insideTicketsTab = false;
let foundTicketsTabStart = false;
let ticketsBlockEndIdx = -1;

for (let i = 0; i < lines.length; i++) {
  if (lines[i].includes("activeTab === 'tickets' && (")) {
    insideTicketsTab = true;
    foundTicketsTabStart = true;
  }
  
  if (insideTicketsTab && lines[i].includes("activeTab === 'reports'")) {
    insideTicketsTab = false;
    ticketsBlockEndIdx = i - 1; // Actually we want to rewrite the whole tickets block
    break;
  }
}

// Instead of manual string manipulation, let's just do a regex replace for the entire tickets block.
const regex = /\{activeTab === 'tickets' && \([\s\S]*?(?=\{activeTab === 'reports' &&)/;
...
if (code.match(regex)) {
  ...
  console.log("Admin tickets block completely replaced.");
} else {
